...
function createSlideHeader(artwork, logo, title, date, subtitle, url) {
  ...
  // Create subtitle wrapper for scrolling
  ...
  setTimeout(function() {
  console.log("Subtitle height:", subtitleElement.scrollHeight);
  console.log("Wrapper height:", subtitleWrapper.clientHeight);
...
// Function to initialize and control subtitle scrolling
function initializeSubtitleScroll(subtitleElement, subtitleWrapper) {
  // Get the heights to determine if scrolling is needed
  var subtitleHeight = subtitleElement.offsetHeight;
  var wrapperHeight = subtitleWrapper.offsetHeight;

  // Only apply scrolling animation if content exceeds wrapper height
  if (subtitleHeight > wrapperHeight) {
    // Calculate the appropriate animation duration based on text length
    var scrollDistance = subtitleHeight - wrapperHeight;
    var scrollDuration = Math.max(scrollDistance * 0.08, 15); // At least 15 seconds, but longer for more text
    
    // Apply the animation
    subtitleElement.style.animation = `scrollText ${scrollDuration}s linear infinite alternate`;
    
    // Start in paused state
    subtitleElement.style.animationPlayState = 'paused';
    
    // Add event listeners to control animation on hover
    ...
    setTimeout(function() {
      ...
      // Pause after completing one cycle
      setTimeout(function() {
        subtitleElement.style.animationPlayState = 'paused';
      }, scrollDuration * 1000);
    }, 2000);
